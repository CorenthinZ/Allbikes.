...
public class FirestoreDb
{
    ...
    @NonNull
    public static Task<QuerySnapshot> readWhere(String collection, String key, Object value,
                                                Callback2<String, Map<String, Object>> onObject)
    {
        return getDb().collection(collection).whereEqualTo(key, value).get().addOnCompleteListener(task ->
        {
           if (!task.isSuccessful())
           {
               Log.w("", "Error getting documents !", task.getException());
           ...
           for (QueryDocumentSnapshot doc : task.getResult())
               onObject.call(doc.getId(), doc.getData());
        });
    }
}
